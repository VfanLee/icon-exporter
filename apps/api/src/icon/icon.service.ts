...
const EXPORT_FOLDER_NAME = 'icons'
...
export class IconService {
  ...
  async exportIcon(dto: ExportIconDto) {
    ...
    const { archive, completed } = this.zipBuilder.createArchive()
    ...
    try {
        ...
        switch (output.format) {
          case 'svg':
            this.zipBuilder.appendBuffer(
              ...
              `${EXPORT_FOLDER_NAME}/${dto.filename}.svg`,
            )
            break
          ...
          default: {
            ...
            for (const size of output.sizes) {
              ...
              this.zipBuilder.appendBuffer(
                archive,
                buffer,
                `${EXPORT_FOLDER_NAME}/${this.buildFileName(dto.filename, size.width, size.height, rasterFormat)}`,
              )
            }
          }
        }
      ...
      return await this.zipBuilder.finalize(archive, completed)
    } catch (error) {
      archive.abort()
